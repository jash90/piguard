...
export function BlocklistPage() {
  ...
  function childName(id?: string) {
    ...
    return (children as ChildProfile[] | undefined)?.find((c) => c._id === id)?.name ?? 'Unknown'
  }
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Manage blocked domains and keywords</p>
        </div>
        <button
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="text-left px-4 py-3 font-medium text-slate-600">Child</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {(filteredRules as BlockRule[]).map((rule) => (
                <tr key={rule._id} className="hover:bg-slate-50">
                  <td className="px-4 py-3 font-mono text-slate-800">{rule.value}</td>
                  ...
                  <td className="px-4 py-3 text-slate-500">{rule.label ?? '—'}</td>
                  <td className="px-4 py-3">
                    <button
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl border border-slate-200 shadow-xl p-6 w-full max-w-md mx-4">
            <h2 className="text-lg font-semibold text-slate-800 mb-4">Add Block Rule</h2>
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">
                  {ruleType === 'domain' ? 'Domain (e.g. example.com)' : 'Value'}
                </label>
                <input
                  type="text"
                  value={domain}
                  ...
                  className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  placeholder={ruleType === 'domain' ? 'example.com' : 'Enter value'}
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">
                  Child Profile <span className="text-slate-400 font-normal">(leave blank for all)</span>
                </label>
                <select
                  value={selectedChild}
